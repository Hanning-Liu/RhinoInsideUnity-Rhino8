...
namespace RhinoInside.Unity
{
  ...
  {
    static readonly string RhinoSystemDir = Registry.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\McNeel\Rhinoceros\8.0\Install", "Path", null) as string ?? string.Empty;
    ...
    public static void Init()
    {
      if (!isLoaded)
      {
        if (string.IsNullOrEmpty(RhinoSystemDir))
        {
                    ...
                    Debug.Log("Unable to found Rhino 8 installed on this computer.\n\nDo you want to download it now?");
                    Application.OpenURL("https://www.rhino3d.com/download/rhino/");
        }
        else
        {
